...
# Part 1, make a dictionary to keep all the words
combine_dictionary = {}
file_list = []
...
for root, dirs, files in os.walk(inputpath):
	...
	for file in files:
		if file.endswith("ham.txt"):
			# create a list for this ham file
			hamfile = ['ham']
			# append words into this ham list
			filepath = os.path.join(root, file)
			with open(filepath,'r', encoding="latin1") as f:
				for line in f:
					for word in line.strip().split():
						hamfile.append(word)		# append to hamfile list
			...
			with open(filepath,'r', encoding="latin1") as f:
				for line in f:
					for word in line.strip().split():
						spamfile.append(word)		# append to spamfile list
						# this counts the number of each word
						if word in combine_dictionary:
							combine_dictionary[word] += 1
						else:
...
# copy a dictionary for avg_weights
avg_dictionary = combine_dictionary.copy()
avg_dictionary = avg_dictionary.fromkeys(avg_dictionary, 0)
...
for counter in range(30):
	...
	for email in file_list:
		...
		if email[0] == 'ham':
			...
			if y_alpha <= 0:
				for word in email[1:]:
					combine_dictionary[word] -= 1	# w = w + yx
					avg_dictionary[word] = avg_dictionary[word] -  1*c		# u = u + ycx
				bias -= 1	# b = b + y
				avg_bias = avg_bias - 1*c	# avg_bias = avg_bias + yc
...
# calculate avg_weight(u)
for word in avg_dictionary:
	avg_dictionary[word] = combine_dictionary[word] - (1/c) * avg_dictionary[word]

# calculate final avg_bias
avg_bias = bias - (1/c) * avg_bias

#print("Bias is " + str(bias))
#print("Average bias is " + str(avg_bias))

# add bias to the dictionary
avg_dictionary['hw_bias'] = avg_bias
avg_dictionary['file_name'] = 'avg_per_output.txt'

# start to write to a text file
text_file = open("per_model.txt", "w", encoding="latin1")
# json.dump(combine_dictionary, text_file)		# Line 1
# text_file.write("\n")
json.dump(avg_dictionary, text_file)			# Line 2
...
print ("avg_per_learn.py running time is " + str(stop - start))
